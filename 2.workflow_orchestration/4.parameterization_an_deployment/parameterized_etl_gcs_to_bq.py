...
def extract_from_gcs(color: str, year: int, month: int) -> Path:
    ...
    logger.info(f"To Path: ../2.gcp_etl_prefect/data/{color}/{color}_tripdata_{year}-{month:02}.parquet")
    path=gcp_cloud_storage_bucket_block.download_object_to_path(
        from_path=gcs_path,
        to_path=f"/Users/prinjaldave/Library/CloudStorage/OneDrive-NortheasternUniversity/[01] Northeastern/[03] Self Learn/Data Engineering/DataTalks Club/dataengineering/2.workflow_orchestration/2.gcp_etl_prefect/data/{color}/{color}_tripdata_{year}-{month:02}.parquet"
    )
